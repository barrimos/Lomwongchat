    ...
    <>
      {
        tickets && tickets.length > 0 ?
          tickets.map((data: reportBubbleTypes, i: number) => {
            return (
              <div className='col-12 col-sm-6 col-lg-4' key={i} >
                <div className='ticketItem' data-ticket={data.ticket} onClick={(e: React.MouseEvent | React.TouchEvent) => onClick(e)}>
                  <div className='collapsibleSwitch'>
                    <h3 className='ticketSN'><span className={`reportStatus ${data.status}`}>{data.status}</span>{data.ticket}</h3>
