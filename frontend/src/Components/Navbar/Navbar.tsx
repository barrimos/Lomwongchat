...
const Navbar = ({ isToggleMenuOpen, toggleMenu, setCurrTab, currTab, ch, setInpNewChannelName, inpNewChannelName, createNewChannel, errorBox, usersOnline, inpSearchUser, setInpSearchUser, joinChannel, handleLogoutBtn, inpSearchChannel, setInpSearchChannel, resultSearchChannel, clearSearchChannel, handleSearching }: Props) => {
  return (
    <nav id='navbar' className='d-md-none'>
      <div className="row no-gutters d-flex justify-content-between align-items-center">
        ...
        <div className='col-6 d-md-none'>
          <select name="channelSelect" id="channelSelect" className='channelSelect' defaultValue='lobby' onChange={(e: React.ChangeEvent<HTMLSelectElement>) => joinChannel(e.target.value)} >
            {
              ch && ch.length > 0 ?
